...
func (f *AllocationFormatter) SupportedTypes() []model.AnalysisDataType {
	return []model.AnalysisDataType{model.DataTypeAllocation}
}
...
func (f *AllocationFormatter) Format(resp *model.AnalysisResponse, log utils.Logger) {
	log.Info("=== Allocation Analysis Results ===")
	log.Info("Task UUID:         %s", resp.TaskUUID)
	log.Info("Task Type:         %s", resp.TaskType.String())
	log.Info("Total Allocations: %d", resp.TotalRecords)
	log.Info("")

	data, ok := resp.Data.(*model.AllocationData)
	if !ok {
	...
	if len(data.ThreadStats) > 0 {
		...
		for i := 0; i < threadCount; i++ {
			...
			log.Info("  Thread: %s, Allocations: %d (%.2f%%)", t.ThreadName, t.Samples, t.Percentage)
		}
		log.Info("")
	}
...
func (f *AllocationFormatter) printOutputFiles(resp *model.AnalysisResponse, log utils.Logger) {
...
func (f *AllocationFormatter) printSuggestions(resp *model.AnalysisResponse, log utils.Logger) {
