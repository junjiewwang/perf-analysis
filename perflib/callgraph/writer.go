...
func (w *XDotWriter) Write(cg *CallGraph, wr io.Writer) error {
...
func (w *DOTWriter) Write(cg *CallGraph, wr io.Writer) error {
	// Write header
	if _, err := fmt.Fprintln(wr, "digraph callgraph {"); err != nil {
		return err
	}

	// Write graph attributes
	if _, err := fmt.Fprintln(wr, "  node [shape=box];"); err != nil {
		return err
	}
	...
	for _, node := range cg.Nodes {
		label := fmt.Sprintf("%s\\n%.2f%%\\n(%.2f%%)", node.Name, node.TotalPct, node.SelfPct)
		if _, err := fmt.Fprintf(wr, "  \"%s\" [label=\"%s\"];\n", node.ID, label); err != nil {
			return err
		}
	}
	...
	for _, edge := range cg.Edges {
		label := fmt.Sprintf("%.2f%%", edge.Weight)
		if _, err := fmt.Fprintf(wr, "  \"%s\" -> \"%s\" [label=\"%s\"];\n",
