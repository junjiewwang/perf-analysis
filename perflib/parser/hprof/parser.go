...
func (p *Parser) Parse(ctx context.Context, r io.Reader) (*HeapAnalysisResult, error) {
	...
	timer := timerPkg.New("HPROF Parse", timerPkg.WithLogger(p.opts.Logger), timerPkg.WithEnabled(p.opts.Logger != nil))
...
func (p *Parser) buildResult(state *parserState, timer perflib.Timer) *HeapAnalysisResult {
