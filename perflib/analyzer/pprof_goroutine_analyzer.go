...
func (a *PProfGoroutineAnalyzer) AnalyzeFromReader(ctx context.Context, req *model.AnalysisRequest, dataReader io.Reader) (*model.AnalysisResponse, error) {
	...
	if taskDir == "" {
		var err error
		taskDir, err = a.EnsureOutputDir("pprof-goroutine-analysis")
		if err != nil {
	...
	distribution, totalCount := analyzeGoroutineDistribution(prof)
...
func analyzeGoroutineDistribution(prof *profile.Profile) ([]model.GoroutineGroup, int64) {
