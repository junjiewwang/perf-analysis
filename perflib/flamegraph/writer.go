...
func (w *FoldedWriter) Write(fg *FlameGraph, wr io.Writer) error {
	return w.writeNode(fg.Root, "", wr)
}

func (w *FoldedWriter) writeNode(node *Node, prefix string, wr io.Writer) error {
	// Build current stack
	currentStack := prefix
	...
	for _, child := range node.Children {
		if err := w.writeNode(child, currentStack, wr); err != nil {
			return err
		}
	}
